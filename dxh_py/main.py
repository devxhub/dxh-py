"""
Main entry point for the `dxh_py` command.

The code in this module is also a good example of how to use dxh_py as a
...
from dxh_py.config import get_user_config
from dxh_py.exceptions import InvalidModeException
from dxh_py.generate import generate_context, generate_files
from dxh_py.hooks import run_pre_prompt_hook
from dxh_py.prompt import choose_nested_template, prompt_for_config
from dxh_py.replay import dump, load
from dxh_py.repository import determine_repo_dir
from dxh_py.utils import rmtree
...
def dxh_py(
    ...
    """
    Run dxh_py just as if using it from the command line.

    :param template: A directory containing a project template directory,
        or a URL to a git repository.
    :param checkout: The branch, tag or commit ID to checkout after clone.
    :param no_input: Do not prompt for user input.
        Use default values for template parameters taken from `dxh_py.json`, user
    ...
    :param directory: Relative path to a dxh_py template in a repository.
    ...
    base_repo_dir, cleanup_base_repo_dir = determine_repo_dir(
        ...
        clone_to_dir=config_dict['dxh_pys_dir'],
        checkout=checkout,
        no_input=no_input,
        password=password,
        directory=directory,
    ...
    context_file = os.path.join(repo_dir, 'dxh_py.json')
    ...
    if replay:
        ...
        items_for_prompting = {
            k: v
            for k, v in context['dxh_py'].items()
            if k not in context_from_replayfile['dxh_py']
        }
        context_for_prompting = {}
        context_for_prompting['dxh_py'] = items_for_prompting
    ...
    # preserve the original dxh_py options
    # print(context['dxh_py'])
    context['_dxh_py'] = {
        k: v for k, v in context['dxh_py'].items() if not k.startswith("_")
    }
    ...
    with import_patch:
        if {"template", "templates"} & set(context["dxh_py"].keys()):
        ...
        if context_for_prompting['dxh_py']:
            context['dxh_py'].update(
    ...
    context['dxh_py']['_template'] = template

    # include output+dir in the context dict
    context['dxh_py']['_output_dir'] = os.path.abspath(output_dir)

    # include repo dir or url in the context dict
    context['dxh_py']['_repo_dir'] = f"{repo_dir}"

    # include checkout details in the context dict
    context['dxh_py']['_checkout'] = checkout
