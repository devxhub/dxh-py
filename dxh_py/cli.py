...
from dxh_py import __version__
...
from dxh_py.log import configure_logger
from dxh_py.main import dxh_py


def version_msg() -> str:
    """Return the dxh_py version, location and Python powering it."""
    python_version = sys.version
    location = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return f"dxh_py {__version__} from {location} (Python {python_version})"
...
def list_installed_templates(
    ...
    """List installed (locally cloned) templates. Use dxh_py --list-installed."""
    config = get_user_config(passed_config_file, default_config)
    dxh_py_folder: str = config['dxh_pys_dir']
    if not os.path.exists(dxh_py_folder):
        click.echo(
            f"Error: Cannot list installed templates. "
            f"Folder does not exist: {dxh_py_folder}"
        )
        sys.exit(-1)

    template_names = [
        folder
        for folder in os.listdir(dxh_py_folder)
        if os.path.exists(
            os.path.join(dxh_py_folder, folder, 'dxh_py.json')
        )
    ]
...
@click.option(
    '--no-input',
    is_flag=True,
    help='Do not prompt for parameters and only use dxh_py.json file content. '
...
@click.option(
    '--directory',
    help='Directory within repo that holds dxh_py.json file '
...
def main(
    ...
    """Create a project from a dxh_py project template (TEMPLATE).
    ...
    try:
        dxh_py(
            ...
            password=os.environ.get('dxh_py_REPO_PASSWORD'),
            directory=directory,
