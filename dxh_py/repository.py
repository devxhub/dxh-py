...
from dxh_py.exceptions import RepositoryNotFound
from dxh_py.vcs import clone
...
def repository_has_dxh_py_json(repo_directory: str) -> bool:
    """Determine if `repo_directory` contains a `dxh_py.json` file.
    ...
    repo_config_exists = os.path.isfile(
        os.path.join(repo_directory, 'dxh_py.json')
    )
...
def determine_repo_dir(
    ...
    :param directory: Directory within repo where dxh_py.json lives.
    :return: A tuple containing the dxh_py template directory, and
    ...
    for repo_candidate in repository_candidates:
        if repository_has_dxh_py_json(repo_candidate):
